...
void CSMain(uint3 tid: SV_DispatchThreadID)
{
    ...
    // Resources
    Texture2D<float> depth = BindlessTexture2DFloat_Load(Push.Depth);
    Texture2D<float4> normal = BindlessTexture2DFloat4_Load(Push.Normal);
    ...
    StructuredBuffer<TileData> tiles = BindlessSRV_TileData_Load(Push.TilesArray);
    StructuredBuffer<uint> bins = BindlessSRV_uint_Load(Push.BinsArray);
    Camera camera = BindlessCBV_Camera_Load(Push.CameraDataSRV);
