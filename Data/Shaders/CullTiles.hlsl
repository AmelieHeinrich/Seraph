...
void CSMain(uint3 tid: SV_DispatchThreadID, uint3 gtid: SV_GroupThreadID, uint3 gid : SV_GroupID)
{
    ...
    StructuredBuffer<PointLight> pointLights = BindlessSRV_PointLight_Load(Push.LightIndex);
    StructuredBuffer<SpotLight> spotLights = BindlessSRV_SpotLight_Load(Push.SpotLightArray);
    RWStructuredBuffer<TileData> tiles = BindlessUAV_TileData_Load(Push.TileArray);
    RWStructuredBuffer<uint> bins = BindlessUAV_uint_Load(Push.BinsArray);
