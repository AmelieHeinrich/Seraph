...
float3 GetNormal(VertexInput input, uint normalIndex, SamplerState sampler)
{
    ...
    Texture2D<float4> texture = BindlessTexture2DFloat4_Load(NonUniformResourceIndex(normalIndex));
...
struct HitData
{
    ...
    void Load(in RayQuery<RAY_FLAG_NONE> q)
    {
...
{
    Camera camera = BindlessCBV_Camera_Load(Push.Camera);
...
void CSMain(uint3 tid: SV_DispatchThreadID)
{
    ...
    Texture2D<float4> albedoTex = BindlessTexture2DFloat4_Load(Push.Albedo);
    Texture2D<float4> normalTex = BindlessTexture2DFloat4_Load(Push.Normal);
    Texture2D<float> depthTex = BindlessTexture2DFloat_Load(Push.Depth);

    RaytracingAccelerationStructure rtAS = BindlessAccelerationStructure_Load(Push.AS);
    StructuredBuffer<SceneInstance> instances = BindlessSRV_SceneInstance_Load(Push.SceneInstances);
    SamplerState sampler = BindlessSampler_Load(Push.Sampler);
    Camera camera = BindlessCBV_Camera_Load(Push.Camera);
