...
void CSMain(uint3 tid : SV_DispatchThreadID)
{
    ...
    RaytracingAccelerationStructure accelerationStructure = BindlessAccelerationStructure_Load(Push.AccelerationStructure);
    StructuredBuffer<VertexInput> vertices = BindlessSRV_VertexInput_Load(Push.VBV);
    StructuredBuffer<uint> indices = BindlessSRV_uint_Load(Push.IBV);
    Texture2D<float4> albedo = BindlessTexture2DFloat4_Load(Push.SRV);
