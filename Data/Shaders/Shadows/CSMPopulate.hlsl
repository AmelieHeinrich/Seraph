...
float CalculateShadowCascade(float4 world, float3 N, float3 L, int layer)
{
    ...
    StructuredBuffer<Cascade> cascades = BindlessSRV_Cascade_Load(Push.CascadeArrayIndex);
    ...
    Texture2D<float> shadowMap0 = BindlessTexture2DFloat_Load(cascade0.SRVIndex);
    Texture2D<float> shadowMap1 = BindlessTexture2DFloat_Load(cascade1.SRVIndex);
    ...
    float shadow0 = PCFCascade(shadowMap0, sampler, world, L, cascade0.View, cascade0.Proj, bias0, kernelSize);
    float shadow1 = PCFCascade(shadowMap1, sampler, world, L, cascade1.View, cascade1.Proj, bias1, kernelSize);
...
void CSMain(uint3 dtid: SV_DispatchThreadID)
{
    ...
    // Load resources
    RWTexture2D<float> output = BindlessRWTexture2DFloat_Load(Push.OutputIndex);
    Texture2D<float> depth = BindlessTexture2DFloat_Load(Push.DepthIndex);
    Texture2D<float4> normal = BindlessTexture2DFloat4_Load(Push.NormalIndex);
    StructuredBuffer<DirectionalLight> sun = BindlessSRV_DirectionalLight_Load(Push.SunIndex);
    StructuredBuffer<Cascade> cascades = BindlessSRV_Cascade_Load(Push.CascadeArrayIndex);
    Camera camera = BindlessCBV_Camera_Load(Push.CameraIndex);
