...
#include "Common/Lights.hlsl"
#include "Common/Math.hlsl"
#include "Common/Compute.hlsl"
...
struct HitData
{
    ...
    void LoadCandidate(in RayQuery<RAY_FLAG_NONE> q)
    {
        ...
        StructuredBuffer<SceneInstance> instances = BindlessSRV_SceneInstance_Load(Push.SceneInstances);
        SceneInstance instance = instances[instanceIndex];
        StructuredBuffer<VertexInput> vertices = BindlessSRV_VertexInput_Load(instance.VertexBuffer);
        StructuredBuffer<uint> indices = BindlessSRV_uint_Load(instance.IndexBuffer);
        StructuredBuffer<SceneMaterial> materials = BindlessSRV_SceneMaterial_Load(instance.MaterialBuffer);

        SceneMaterial material = materials[instance.MaterialIndex];
        Texture2D<float4> albedo = BindlessTexture2DFloat4_Load(material.Albedo);
...
void CheckTransparency(inout RayQuery<RAY_FLAG_NONE> q)
{
    HitData data;
...
void CSMain(uint3 dtid: SV_DispatchThreadID)
{
    ...
    // Load resources
    RWTexture2D<float> output = BindlessRWTexture2DFloat_Load(Push.Output);
    Texture2D<float> depth = BindlessTexture2DFloat_Load(Push.GBufferDepth);
    Texture2D<float4> normal = BindlessTexture2DFloat4_Load(Push.GBufferNormal);
    RaytracingAccelerationStructure sceneAS = BindlessAccelerationStructure_Load(Push.SceneAS);
    StructuredBuffer<DirectionalLight> sun = BindlessSRV_DirectionalLight_Load(Push.SunArray);
    Camera camera = BindlessCBV_Camera_Load(Push.CameraIndex);
