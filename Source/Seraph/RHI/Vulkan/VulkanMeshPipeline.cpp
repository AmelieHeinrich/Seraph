...
// > Create Time: 2025-05-31 19:46:23
//
...
VkCompareOp VulkanMeshPipeline::ToVkCompareOp(RHIDepthOperation op)
{
    switch (op)
    {
...
VkCullModeFlagBits VulkanMeshPipeline::ToVkCullMode(RHICullMode mode)
{
    switch (mode)
    {
...
VkPolygonMode VulkanMeshPipeline::ToVkFillMode(RHIFillMode mode)
{
    switch (mode)
    {
...
VulkanMeshPipeline::VulkanMeshPipeline(VulkanDevice* device, RHIGraphicsPipelineDesc desc)
    : mParentDevice(device)
{
    ...
    ASSERT_EQ(result == VK_SUCCESS, "Failed to create Vulkan mesh pipeline!");
