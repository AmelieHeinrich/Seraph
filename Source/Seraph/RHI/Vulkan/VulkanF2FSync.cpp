...
// > Create Time: 2025-05-29 18:56:43
//

#include "VulkanF2FSync.h"
#include "VulkanDevice.h"

VulkanF2FSync::VulkanF2FSync(VulkanDevice* device, VulkanSurface* surface, VulkanCommandQueue* queue)
    : mParentDevice(device), mSurface(surface), mCommandQueue(queue)
{
    ...
    for (int i = 0; i < FRAMES_IN_FLIGHT; i++) {
        ...
        ASSERT_EQ(result == VK_SUCCESS, "Failed to create image available semaphore!");
        
        result = vkCreateSemaphore(device->Device(), &semaphoreInfo, nullptr, &mImageRenderedSemaphore[i]);
        ASSERT_EQ(result == VK_SUCCESS, "Failed to create image rendered semaphore!");
...
uint VulkanF2FSync::BeginSynchronize()
{
    ...
    ASSERT_EQ(result == VK_SUCCESS, "Failed to get next swapchain image!");
...
void VulkanF2FSync::EndSynchronize(IRHICommandBuffer* submitBuffer)
{
    ...
    ASSERT_EQ(result == VK_SUCCESS, "Failed to end F2F synchronization!");
}

void VulkanF2FSync::PresentSurface()
{
    ...
    ASSERT_EQ(result == VK_SUCCESS, "Failed to present F2F surface!");
