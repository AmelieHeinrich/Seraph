...
D3D12ComputePipeline::D3D12ComputePipeline(D3D12Device* device, RHIComputePipelineDesc desc)
{
    ...
    Desc.CS.pShaderBytecode = desc.ComputeBytecode.Bytecode.data();
    Desc.CS.BytecodeLength = desc.ComputeBytecode.Bytecode.size();
    ...
    ASSERT_EQ(SUCCEEDED(result), "Failed to create D3D12 compute pipeline state!");

    SERAPH_WHATEVER("Created D3D12 compute pipeline state!");
}
